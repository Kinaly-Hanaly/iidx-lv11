function display_table(csv_tracks) {
  ...
  csv_tracks.forEach((item, i) => {
    ...
    if (item['your_lamp'] == 'HARD CLEAR') {
      $('#track-' + item['code']).addClass('table-danger');
      hard_tracks_num += 1;
      $('#tile-' + item['code']).addClass('bg-danger');
    }else if (item['your_lamp'] == 'EX HARD CLEAR') {
  ...
  $('#your-hard-num').text(hard_tracks_num + exh_tracks_num + fc_tracks_num);
  if (csv_tracks.length > 0) {
    $('#your-hard-rate').text(Math.floor(((hard_tracks_num + exh_tracks_num + fc_tracks_num) / table_array.length) * 100 * 10) / 10);
  } else {
    $('#your-hard-rate').text('---');
  }
  $('#progress-hard').text(hard_tracks_num);
  $('#progress-exh').text(exh_tracks_num);
  $('#progress-fc').text(fc_tracks_num);
  if (csv_tracks.length > 0) {
    $('#progress-hard').animate({
      width: (hard_tracks_num / table_array.length) * 100 + '%'
    }, 'fast');
